				...
				<div class="col-lg-3 col-md-4 col-sm-12">
					...
					<div class="uprofile-info">
						<ul class="list-unstyled">
							...
							<li><i class='fa fa-users'></i>User(s):  {{App\model\Users\UserInfo::where(['status' => 'user','dealerid' => $user->dealerid])->count()}} </li>
							@endif
							...
							@endif
							@if($user->status == 'manager')
							<li><i class='fa fa-user-tie'></i> {{App\model\Users\UserInfo::where(['status' => 'reseller','manager_id' => $user->manager_id])->count()}} Resellers</li>
							<li><i class='fas fa-user-friends'></i> {{App\model\Users\UserInfo::where(['status' => 'dealer','manager_id' => $user->manager_id])->count()}} Contractors</li>
							@endif
						</ul>
					</div>
					@if(Auth::user()->status != "support")
					<div class="uprofile-buttons">
						<button class="btn btn-success enable" id="disableBtn" value="enable" onclick="toggle('{{$user->status}}');" style="width: 100%;">This {{ucwords($user->status)}} is Enable</button>
					</div>
					@endif
				</div>
