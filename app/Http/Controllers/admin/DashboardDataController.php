...
class DashboardDataController extends Controller
{
    ...
    {
        //
        $onlineUser = RadCheck::where('status', 'user')
            ->where('attribute', 'Cleartext-Password')
            ->whereIn('username', function ($query) {
                $query->select('username')->from('radacct')->where('acctstoptime', null);
            })
            ->count();
        //
    ...
    public function upcoming_expire_consumer()
    {
        ...
        $upcoming_expiry_users = DB::table('user_info')->join('user_status_info', 'user_status_info.username', '=', 'user_info.username')->where('user_status_info.card_expire_on', '>', NOW())->where('user_status_info.card_expire_on', '<', $seven_days)->where($whereArray)->where('user_info.status', '=', 'user')->count();
        //
    ...
    public function verified_mobile()
    {
        //
        $whereArray = $this->where_info();
        $mobVerify = UserInfo::where($whereArray)
            ->where('status', 'user')
            ->whereIn('username', function ($query) {
                $query->select('username')->from('user_verification')->where('mobile', '!=', '');
            })
            ->select('username')
            ->count();
        //
    ...
    public function verified_cnic()
    {
        //
        $whereArray = $this->where_info();
        $verified_users = UserInfo::where($whereArray)
            ->where('status', 'user')
            ->whereIn('username', function ($query) {
                $query->select('username')->from('user_verification')->where('cnic', '!=', '');
            })
            ->select('username')
            ->count();
        //
    ...
    {
        $whereArray = $this->where_info();
        $expired_users = DB::table('user_info')->join('user_status_info', 'user_status_info.username', '=', 'user_info.username')->where('user_status_info.card_expire_on', '>', '2000-01-01')->where('user_status_info.card_expire_on', '<', date('Y-m-d'))->where($whereArray)->where('user_info.status', '=', 'user')->count();
    ...
    public function suspicious_consumer()
    {
        ...
        $whereArray = $this->where_radcheck();
        //
        $suspiciousUser = RadCheck::where('status', 'user')
            ->where('attribute', 'Cleartext-Password')
            ->where($whereArray)
            ->whereNotIn('radcheck.username', function ($query) {
                $query->select('radacct.username')->from('radacct')->where('radacct.acctstoptime', null);
            })
        ...
        foreach ($suspiciousUser as $sus) {
            $hourdiff = '';
            $lastLoginDetail = RadAcct::where('username', $sus->username)
