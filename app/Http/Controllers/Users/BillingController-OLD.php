...
class BillingController extends Controller
{
 	...
 	switch($status){
		...
		// dd(count($no_of_packages));
 		...
 		case "recieve" : {
 			...
 			if($status == 'inhouse' && Auth::user()->resellerid == NULL){
 				return view('users.billing.manager_support.recieve_amount');
 			}else{ 
 ...
 public function serverSideCashReceipt()
 {
 	...
 	->addColumn("senders", function ($row) {
 		...
 		if(Auth::user()->status == "reseller" || Auth::user()->username == "account09" || Auth::user()->username == "account06"){
 			if($row->status == "POST"){
 				$status = $row->status;
 			}else{
 				$csrf = csrf_token();
...
private function storeNoneCash(Request $request, $status){
	...
	*/
	...
	public function manager_support_transfer_amount(request $request){
		//
		$transferAmount = $request->get('amount');
		$usernameReciever = $request->get('username');
		$comment = $request->get('comment');
		$usernameSender = Auth::user()->username;
		$lastRemainingAmount = 0.00;
		$sender = 'manager';
		//
		if(empty(Auth::user()->username)){
			...
			Session()->flash("error", "Your panel has been freezed");
			return back();      
		}if(empty($usernameReciever) || empty($transferAmount) ){
			Session()->flash("error", "Some data missing");
			return back(); 
		}
		//
		$userReciever = UserInfo::where(['username' =>$usernameReciever])->first();
		...
		//
		$discount = $userAmount->discount;
		$credit_limit = $userAmount->credit_limit;
		$transferAmount = $transferAmount / $discount;
		$lastRemainingAmount  = $userAmount->amount;
		//
		if($transferAmount > $credit_limit){
			Session()->flash("error", "Tranfer amount is greater than credit limit");
			return back(); 
		}
		//
		if($userReciever->status == 'dealer'){
			//
			$sender = $userReciever->resellerid;
			$senderUserAmount = UserAmount::where("username",$userReciever->resellerid)->first();
			//
			if($senderUserAmount->amount < $transferAmount){
				Session()->flash("error", "Sender don`t have enough amount in wallet");
				return back(); 
			}
		}
		// Add Amount in wallet
		$userAmount->amount = $userAmount->amount + $transferAmount;
		$userAmount->save();
		//
		// deduct amount from wallet
		if($userReciever->status == 'dealer'){
			//
			$senderUserAmount->amount =  $senderUserAmount->amount - $transferAmount;
			$senderUserAmount->save();
		}
		//
		// amount transaction
		$transactionAmount = new AmountTransactions();
		$transactionAmount->receiver = $usernameReciever;
		$transactionAmount->sender = $sender; 
		$transactionAmount->amount = $transferAmount;
		$transactionAmount->last_remaining_amount = $lastRemainingAmount;
		$transactionAmount->date = Date('Y-m-d H:i:s');
		$transactionAmount->action_by_user = $usernameSender; 
		$transactionAmount->action_ip = $request->ip();
		$transactionAmount->comments = $comment;
		$transactionAmount->commision = 'no';
		$transactionAmount->type = 'transfer';
		$transactionAmount->save();
		//
		Session()->flash("success", "Amount Transfered Successfully");
		return back();
		//
	}
	//////////////////////////////////////////
	public function manager_support_receive_amount(request $request){
		// 
		if(empty(Auth::user()->username)){
			session()->flash('error','Session Expired');
			return back(); 
		...
		//
		$usernameReciever = (empty($request->get('username'))) ? $request->get('resellerid') : $request->get('username');
		$recieveAmount = $request->get('amount');
		$paidBy = $request->get('paidBy');
		$usernameSender = Auth::user()->manager_id;
		$comment = $request->get('comment');
		$cash_amount = $request->get('cash_amount');
		$paymentType = $request->get('paymentType');
		if($paymentType == 'bank'){
		...
		if($cash_amount == "yes"){
		//
			$totalTransactionamount = AmountTransactions::where('receiver',$usernameReciever)->where('commision','!=','yes')->sum('amount');
			$totalTransactionrevert = AmountTransactions::where('receiver',$usernameReciever)->sum('cash_amount');
			$totalTransaction = $totalTransactionamount - $totalTransactionrevert;
    	///
			if($recieveAmount > $totalTransaction ){
    		//
				session()->flash('error','Your Transfer is less then you recieve');
				return back();
    		//
		...
		//
		if($cash_amount == "no"){
			$userAmount->amount +=  $recieveAmount;
			$userAmount->save();
		}
		//
		$userAccount = UserAccount::where('username',$usernameSender)->first();
		...
		$lastDebit = 0.0;
        //
		$data = UserInfo::where('username',$usernameReciever)->first(); 
        //
		$userAccount = new UserAccount();
		$userAccount->username = $usernameSender;
		$userAccount->status = 'manager';
		$userAccount->debit = $adjustedAmount;
		$userAccount->action_by = Auth::user()->username; 
		$userAccount->action_ip = $request->ip();
		$userAccount->action_at = Date('Y-m-d H:i:s');
		$userAccount->save();
		//////////////////////////////////////////////////////////
		// payment transation
		$paymentsTransaction = new PaymentsTransactions();
		$paymentsTransaction->sender = $usernameReciever;
		if($data->status == 'reseller'){
			$paymentsTransaction->receiver = 'manager';
		}else{
			$paymentsTransaction->receiver = $data->resellerid;	
		}
        ...
        if($cash_amount == "yes"){
        	$transactionAmount = new AmountTransactions();
        	$transactionAmount->sender = 'manager'; 
        	$transactionAmount->receiver = $usernameReciever; 
        	$transactionAmount->amount = 0;
        	$transactionAmount->cash_amount = $recieveAmount;
        	$transactionAmount->last_remaining_amount = 0;
        	$transactionAmount->date = Date('Y-m-d H:i:s');
        	$transactionAmount->action_by_user = Auth::user()->username; 
        	$transactionAmount->action_ip = $request->ip();
        	$transactionAmount->comments = $comment;
        	$transactionAmount->commision = 'no';
        	$transactionAmount->com_amount = 0;
