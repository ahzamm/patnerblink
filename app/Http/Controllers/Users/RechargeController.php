...
class RechargeController extends Controller
{
 ...
 public function single($id)
 {
 	...
 	if(($data['info']->company_rate == 'yes')  && ($data['info']->profile_amount != $profileData->base_price)){
 		session()->flash('error','Profile rate has been changed, kindly update your consumer rate once.');
 		return back();
 	}
 	//
 				...
 				if (!empty($freeze_account)) {
					...
					if ($amount >= $profileRate['rate']) {
						...
						if ( ($get_user_info->company_rate == 'yes') && ($dasti_amountFromDealerproRate->base_price > $base_price_user) ) {
							...
							// return redirect()->route('users.user.index1', ['status' => 'user']);
							return back();
						}if ( ($get_user_info->company_rate != 'yes')  && ($dasti_amountFromDealerproRate->base_price_ET > $base_price_user) ) {
							session()->flash('error', 'Consumer profile rate must be greater than dealer Rate.');
							// return redirect()->route('users.user.index1', ['status' => 'user']);
							return back();
						}
					...
					///////////////////////// check wallet deduction ///////////////////////
						...
						if(empty($expire_user)){
							session()->flash('error', 'Some entry missing');
							return back();
						}
						//
	...
	//
 	...
 	// 
						...
						try {
							//
							////////// WALLET DEDUCTION ////////////////
							$userAmount = UserAmount::where('username', $currentUserResellerUserName->username)->lockForUpdate()->first();
...
// $charge = strtotime('11:58:59');
	...
	public function recharge_it(request $request){
		...
		if(($userInfo->company_rate == 'yes')  && ($userInfo->profile_amount != $profileRate->base_price)){
			return abort(403, 'Error : Profile rate has been changed, kindly update your consumer rate once.'); 
		}
        /////////////////////////////////////////////////////////////////////////////////////
        /////////////////////////// RECHARGING /////////////////////////////////////////////
        ///////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //
		...
		if($resellerprofileRate->allow_auto_profit == 'yes'){
			...
			$autoProfitData = $this->calculate_auto_profit($resellerprofileRate,$dealerprofileRate,$profileRate,$userInfo->company_rate);
	...
	public function calculate_auto_profit($reseller_pr,$dealer_pr,$subdealer_pr,$company_rate){
		// defualt values
		$data['reseller_profit'] = $data['dealer_profit'] = 0;
		// if has CVAS
		$data['reseller_profit'] = $dealer_pr->base_price_ET - $reseller_pr->rate;
		if(Auth::user()->status == "subdealer"){
			$data['dealer_profit'] = $subdealer_pr->base_price_ET - $dealer_pr->base_price_ET;
		}
		// if dont have CVAS
		...
		if($resellerInfo->has_license <= 0){
			//
			$data['resellerTaxes'] = $this->get_tax_sum($reseller_pr->rate,$company_rate);
			$data['dealerTaxes'] = $this->get_tax_sum($dealer_pr->base_price_ET,$company_rate);
			if(Auth::user()->status == "subdealer"){
				$data['subdealerTaxes'] = $this->get_tax_sum($subdealer_pr->base_price_ET,$company_rate);
			}
			//
			$data['reseller_profit']  = $data['reseller_profit'] - ($data['dealerTaxes']['sum'] - $data['resellerTaxes']['sum'] );
			//
			if(Auth::user()->status == "subdealer"){
				$data['dealer_profit']  = $data['dealer_profit'] - ($data['subdealerTaxes']['sum'] - $data['dealerTaxes']['sum'] );
			}
			//
		}
	...
	public function get_tax_sum($p_rate,$company_rate){
