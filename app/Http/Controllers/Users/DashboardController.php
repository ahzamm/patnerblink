...
//
  ...
  if(Auth::user()->status == 'dealer'){
    $checkVerificationRestriction = DealerProfileRate::where(['dealerid' => Auth::user()->dealerid])->select('verify')->first();
 //
    ...
    $checkVerificationRestriction = SubdealerProfileRate::where(['sub_dealer_id' => Auth::user()->sub_dealer_id])->select('verify')->first();
 //
    if(!empty($checkVerificationRestriction) && $checkVerificationRestriction->verify == 'no'){
        $verifyRestricted = 'Not Restricted';
    }
...
if($status == "manager")
{
   ...
   $profileCollection = ManagerProfileRate::where(['manager_id' => Auth::user()->manager_id])->orderBy('groupname')->get();
           // for online user
...
//
 ...
 //
  ...
  // 
   ...
   $profileCollection = ResellerProfileRate::where(['resellerid' => Auth::user()->resellerid])->orderBy('groupname')->get();
           // for online user
   // $activeUser  = DB::table('user_info')
   // ->join('user_status_info', 'user_status_info.username', '=', 'user_info.username')
...
//
 ...
 //////// charge never expire users /////////////////
           ...
           //$activeUsers = $currentUser->user_status_info;
   $profileCollection = DealerProfileRate::where(['dealerid' => Auth::user()->dealerid])->orderBy('groupname')->get();
           // for online user
   // $activeUser  = DB::table('user_info')
   // ->join('user_status_info', 'user_status_info.username', '=', 'user_info.username')
   // ->where('user_status_info.expire_datetime', '>', DATE('Y-m-d H:i:s'))
...
//
 ...
 //
   ...
   $currentUser = UserInfo::find($id);
          ...
          //$activeUsers = $currentUser->user_status_info;
   $profileCollection = SubdealerProfileRate::where(['sub_dealer_id' => Auth::user()->sub_dealer_id])->orderBy('groupname')->get();
             // for online user
   // $activeUser  = DB::table('user_info')
   // ->join('user_status_info', 'user_status_info.username', '=', 'user_info.username')
   // ->where('user_status_info.expire_datetime', '>', DATE('Y-m-d H:i:s'))
...
//
 ...
 //
    ...
    $domainDetails = Domain::where('resellerid' , Auth::user()->resellerid)->first();
    //
    $download = RadAcct::select("acctoutputoctets","acctstarttime")->where("acctstarttime",">=",date("Y-m-01 00:00:00"))->where("acctstarttime","<=",date("Y-m-t 00:00:00"))->where(["username" => Auth::user()->username])->get();
    $upload = RadAcct::select("acctinputoctets","acctstarttime")->where("acctstarttime",">=",date("Y-m-01 00:00:00"))->where("acctstarttime","<=",date("Y-m-t 00:00:00"))->where(["username" => Auth::user()->username])->get();
    $invoice = AmountBillingInvoice::where('username',Auth::user()->username)->orderBy('date','DESC')->take(24)->get();
    ...
    {
      $get_parent_data = UserInfo::where('trader_id',$get__user_data->trader_id)->where('status','trader')->first();

  }elseif($get__user_data->sub_dealer_id != Null)
  {
      $get_parent_data = UserInfo::where('sub_dealer_id',$get__user_data->sub_dealer_id)->where('status','subdealer')->first();
  }else{
     $get_parent_data = UserInfo::where('dealerid',$get__user_data->dealerid)->where('status','dealer')->first();   
 }
...
$userStatus = DB::table('user_info')
->where('disabled_expired', '!=', 'YES')
->count();
...
{   
    ...
    //
    $invalidLogin = RadCheck::where('status','user')->where('attribute','Cleartext-Password')->where($whereRadiusArray)->whereIn('username', function($query){
        $query->select('username')
        ->from('radpostauth');
...
public function charge_never_expire($currentdealerid){
  ...
  $getUser  = DB::table('user_info')
  ->join('user_status_info', 'user_status_info.username', '=', 'user_info.username')
  ->join('never_expire','never_expire.username','user_info.username')
  ->where('user_status_info.card_expire_on', '<=', date('Y-m-d'))
  ->where('user_info.status','=','user')->where('user_info.profile','!=', NULL)->where('user_info.profile','!=', 'DISABLED')->where('user_info.profile','!=', 'EXPIRED')->where('user_info.manager_id','=',$currentdealerid)
  ->where('user_info.never_expire','=','yes')->where('never_expire.todate' ,'>=', date('Y-m-d'))
  ->get();
  // 
  // 
  ...
  // $profile=str_replace('k', '', $profile);
   ...
   $data=array('username' => $username,'profileGroupname' => $profile,'ipaddress' => $ip);
// var_dump($data);
...
{
  $getExpiredDate  = DB::table('user_info')
  ->join('never_expire','never_expire.username','user_info.username')
  ->where('user_info.status','=','user')
  ->where('user_info.never_expire','=','yes')->where('never_expire.todate' ,'<', date('Y-m-d'))
  ->get();
