...
public function count_ids($dealerid,$subdealerid,$year,$month,$when){
	...
	->where('user_info.status','=','user');
  	//
	$query2 = $query->whereIn('username', function($subquery1) use ($start,$end){
		$subquery1->select('username')
		->from('amount_billing_invoice')
		->where('amount_billing_invoice.date','>=',$start)
		->where('amount_billing_invoice.date','<=',$end);
	});
	//
	$query3 = $query2->whereIn('username', function($subquery2) use ($start,$end){
		$subquery2->select('username')
		->from('pta_reported_check_date')
		->where('date','<',$end);
	})->get();
	//
	return $query3;
...
public function download_sheet(request $request){
	...
	$dealer_data = $request->get('dealer_data');  
	//
	$year = $request->get('year');   
	$month = $request->get('month');
	//
...
//////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////
	$lineData = array( 'Date', 'Sales Tax Inv.#', 'Billing Inv.#', 'Customer Name & Address', 'CNIC/NTN', 'Location', 'Package', 'TIP', 'FLL', 'CVAS', 'Total Charges(SST)', 'S.S.Tax', 'Total Charges(ADV)', 'Adv.I.Tax', 'Total TAX.', 'Total Amt', 'Contractor ID', 'Trader ID', 'Cust ID','PTA Check Date');
	...
	if(!empty($reseller_data)){
		$rows->where('user_info.resellerid',$reseller_data);
	}if(!empty($dealer_data)){
		$rows->where('user_info.dealerid',$dealer_data);	
	}
	//
	$rows->where('user_info.pta_reported',1)
	->where('user_info.status','=','user')
	->where('amount_billing_invoice.date','>=',$start)
	->where('amount_billing_invoice.date','<=',$end)
	->where('user_info.status','=','user');
	...
	foreach ($query as $row) {
		...
		// 	
		$checkDate = DB::table('pta_reported_check_date')->where('username',$row->username)->first();
		//
		$lineData = array( $row->date, '', '', $nameNaddress, $row->nic, 'Location', $pkg[1], $row->tip_charges, $row->fll_charges, $row->cvas_charges, $totalChargesSST, $row->sst, $totalChargesADV, $row->adv_tax, $totaltaxes, $totalAmount, $row->dealerid, $row->sub_dealer_id, $row->username,@$checkDate->date);
...
public function bill_register_update_data(request $request){
	...
	foreach($id as $key => $value){
		...
		//
		if($userNewCount < $userCurrOldcount){
			...
			foreach($profileRate as $proVal){
				...
				for($i=0; $i<$userCountDiff; $i++){
					...
					if($billing){
						...
						$forUpdateUser->save();
						//
						DB::table('pta_reported_check_date')->insert(['username' => $billing->username, 'date' => $start]);
						//
						$count++;
						$totalCount++;
						//
