        ...
        $expiry_users = DB::table('user_info')
            ...
            ->when(!empty($searchFilter), function ($query) use ($searchFilter) {
                $query->where(function ($subQuery) use ($searchFilter) {
                    $subQuery
                        ->where('user_info.username', 'LIKE', '%' . $searchFilter . '%')
            ...
            ->select([DB::raw('ROW_NUMBER() OVER(ORDER BY user_status_info.card_expire_on ASC) AS serial'), 'user_info.username', DB::raw('CONCAT(user_info.firstname, " ", user_info.lastname) AS fullname'), 'user_info.mobilephone', 'user_info.address', DB::raw('DATEDIFF(user_status_info.card_expire_on, CURDATE()) AS remaining_days'), 'user_info.name AS profile', 'user_status_info.card_charge_on', 'user_status_info.card_expire_on', 'user_info.dealerid', DB::raw('IFNULL(user_info.sub_dealer_id, "N/A") AS sub_dealer_id')]);
